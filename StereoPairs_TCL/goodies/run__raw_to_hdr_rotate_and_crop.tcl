# run__raw_to_hdr_rotate_and_crop.tcl - a "sourceable" file that runs raw_to_hdr.tcl on L/ and R/ subdirectories
...
proc _load_raw_conversion_preferences {} {
...
if { 0 == [_load_raw_conversion_preferences] }  {  ; # unless defined by preferences
...
# (5) Load orientation spec from preferences and decide on rotation and crop parameters
if { 0 == [_set_rotcrop_params_from_preferences \
                          angleL angleR xyRatio padX padY] }  {
  return  0;  # error already printed
}

#~ # (6) Execute the main procedure of "raw_to_hdr.tcl" script
...
# (6) Choose the ultimate per-session temporary directory .
...
# (7) If input white-balance override file exists, tell to use it for L/ directory
...
# (8) Execute the main procedure of "raw_to_hdr.tcl" script in L/ subdirectory
...
# (location of tool-path file reflects Dualcam-Companion software structure)
if { 0 == [raw_to_hdr_main "-inp_dirs {L} -out_subdir_name OUT $TMP_DIR_ARG__OR_EMPTY -final_depth 8 -raw_ext ARW  -rotate $angleL -crop_ratio $xyRatio -pad_x $padX -pad_y $padY  -wb_out_file wb_left.csv $INP_WB_OVRD   -tools_paths_file [dualcam_find_toolpaths_file 0] -do_skip_existing 1 -do_abort_on_low_disk_space 1 -do_preview $previewArg"]}   {
  return  0;  # error already printed
}


# (9) Change image-file names in WB-file created while processing left directory
...
# (10) Execute the main procedure of "raw_to_hdr.tcl" script in R/ subdirectory
...
# (location of tool-path file reflects Dualcam-Companion software structure)
if { 0 == [raw_to_hdr_main "-inp_dirs {R} -out_subdir_name OUT $TMP_DIR_ARG__OR_EMPTY -final_depth 8 -raw_ext ARW -rotate $angleR -crop_ratio $xyRatio -pad_x $padX -pad_y $padY  -wb_out_file wb_right.csv -wb_inp_file wb_ovrd_right.csv  -tools_paths_file [dualcam_find_toolpaths_file 0] -do_skip_existing 1 -do_abort_on_low_disk_space 1 -do_preview $previewArg"] }   {
  return  0;  # error already printed
}
