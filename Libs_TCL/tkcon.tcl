...
proc ::tkcon::InitUI {title} {
    ...
    $w.mbar configure -activeborderwidth 0 -bd 0 -font 7x13b
...
proc ::tkcon::InitTab {w} {
    ...
    foreach col {prompt stdout stderr stdin proc warning info message error fatal} {
	...
	if {$code} {
		...
		if {$OPT(hoterrors)} {
		    ...
		    $w insert output $res\n stderr
                ...
                set res_length [string length $res]
                if {$res_length>$OPT(MAX_ABSOLUTE)} {
		  if [tk_dialog $PRIV(base).warning "Output too large!" \
                       "Output is too large! Are you sure you want to display it on the console?" \
		       questhead 1 "Yes" "No"] {
                     Prompt
                     set PRIV(event) [EvalSlave history nextid]
                     return
                ...
                foreach line [split $res \n] {
                  ...
                  if {$length>$long_line} {set long_line $length}
                }
                if {$long_line>$OPT(MAX_LINE)} {
                  set msg "RESULT CONTAINS A VERY LONG LINE!!! ($long_line bytes)\n"
                  append msg "IT WILL BE SENT TO A SEPARATE EDIT WINDOW\n"
                  append msg "LINES MORE THAN 2048 CHARS WILL BE BROKEN IF POSSIBLE\n"
                  $w insert output $msg info
                  if {$res_length<$OPT(MAX_CHARS)} {
                    set res [::tkcon::BreakLines $res] 
                  }
                  edit -type default -- $res
                } else {
                  uplevel #0 [list set _ $res]
  	          $w insert output $res\n stdout
                }
	    }
	}
...
proc ::tkcon::BreakLines {str {max 2048}} {
  set res ""
  foreach line [split $str \n] {
    set n [string length $line]
    set M [expr ($n/$max)+1]
    for {set i 0} {$i<$M} {incr i} {
      append res "[string range $line [expr $i*$max] [expr ($i+1)*$max-1]]\n"
    }
  }
  return $res
}
...
proc edit {args} {
	...
	grid rowconfigure $w 0 -weight 1
        $w.mbar configure -activeborderwidth 0 -bd 0 -font {Helvetica 11 bold}
    } else {
...
proc ::tkcon::Bindings {} {
	...
	<<TkCon_PreviousSearch>>	<Control-Up>
	<<TkCon_Next>>		<Down>
